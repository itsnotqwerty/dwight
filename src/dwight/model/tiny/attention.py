...
def apply_partial_rope(x: torch.Tensor, freqs: torch.Tensor, rope_dims: int) -> torch.Tensor:
    rotated = x[..., :rope_dims]
    passthrough = x[..., rope_dims:]
    rotated_complex = torch.view_as_complex(rotated.float().reshape(*rotated.shape[:-1], -1, 2))
...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer("freqs", precompute_freqs(rope_dims, max_seq_len), persistent=False)
