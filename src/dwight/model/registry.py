...
MODEL_REGISTRY: dict[str, ModelEntry] = {
    "dwight": ModelEntry(GPTModel, ModelConfig, os.path.join("checkpoints", "model.pt")),
    "tiny": ModelEntry(TinyModel, TinyModelConfig, os.path.join("checkpoints", "tiny.pt")),
}
...
def load_model(model_id: str, device: torch.device) -> tuple[torch.nn.Module, object, str]:
    ...
    if os.path.exists(checkpoint_path):
        ...
        print(f"No checkpoint found for {model_id} – starting with random weights (device: {device}).")
    model.to(device)
    model.eval()
